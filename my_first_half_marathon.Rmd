---
title: "my first half marathon and its analysis"
author: "stewarta"
date: "13 May 2018"
...
There are `r nrow(tbl_df)` total people who ran the half marathon and crossed the finish line according to my data. There were `r nrow(sqldf("select Sx from tbl_df where Sx='F'"))` women  constituting `r nrow(sqldf("select Sx from tbl_df where Sx='F'"))/nrow(tbl_df) *100` % of all runners and `r nrow(sqldf("select Sx from tbl_df where Sx='M'"))` making up the remaining `r nrow(sqldf("select Sx from tbl_df where Sx='M'"))/nrow(tbl_df) *100` %. I actually heard an old woman spectating making the comment "there are lots of women here!" as I was passing by. And even though we're a little way from 50/50, the percentage of women racing is increasing year after year. It reminds me of a comment from Laz, the director of the Barkley Marathons, reportedly saying "it's the year of women", even though the percentages were far from equal. This next graph shows the difference pretty clearly as well as giving us the numbers per category:
